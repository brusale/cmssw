...
SimBarrelValidator::SimBarrelValidator(const edm::ParameterSet& pset)
  ...
  hitMap_ = consumes<std::unordered_map<DetId, float>>(edm::InputTag("simBarrelRecHitMapProducer"));
  ...
  histoProducerAlgo_ = std::make_unique<SimBarrelVHistoProducerAlgo>(psetForHistoProducerAlgo);

  dirName_ = pset.getParameter<std::string>("dirName");
}

SimBarrelValidator::~SimBarrelValidator() {}

void SimBarrelValidator::bookHistograms(DQMStore::IBooker& ibook,
                                    edm::Run const&,
  ...
  if (doSimClustersPlots_) {
    ...
    for (unsigned int ws = 0; ws < label_clustersmask.size(); ws++) {
      ...
      LogDebug("SimBarrelValidator") << dirName << "\n";
      if (!algo.label().empty())
  ...
  for (unsigned int www = 0; www < label_tst.size(); www++) {
    ...
    if (doTrackstersPlots_) {
      ...
      histoProducerAlgo_->bookTracksterSTSHistos(
          ibook, histograms.histoProducerAlgo, SimBarrelVHistoProducerAlgo::validationType::Linking);
      // SimTracksters Pattern Recognition
      ibook.setCurrentFolder(dirName + "/" + label_TSToSTSPR_);
      histoProducerAlgo_->bookTracksterSTSHistos(
          ibook, histograms.histoProducerAlgo, SimBarrelVHistoProducerAlgo::validationType::PatternRecognition);
    }
  }  //end of booking Tracksters loop
}

void SimBarrelValidator::cpParametersAndSelection(const Histograms& histograms,
...
void SimBarrelValidator::dqmAnalyze(const edm::Event& event,
                                const edm::EventSetup& setup,
                                const Histograms& histograms) const {
  using namespace reco;

  LogDebug("SimBarrelValidator") << "\n===================================================="
                             << "\n"
                             << "Analyzing new event"
                             << "\n"
                             << "====================================================\n"
  ...
  edm::Handle<std::unordered_map<DetId, float>> hitMapHandle;
  event.getByToken(hitMap_, hitMapHandle);
  const std::unordered_map<DetId, float>* hitMap = &*hitMapHandle;
  ...
  for (unsigned int scId = 0; scId < nSimClusters; ++scId) {
    if (simClusters[scId].g4Tracks()[0].eventId().event() != 0 or
      ...
      LogDebug("SimBarrelValidator") << "Excluding SimClusters from event: "
