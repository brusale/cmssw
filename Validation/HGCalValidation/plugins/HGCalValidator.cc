...
void HGCalValidator::dqmAnalyze(const edm::Event& event,
  ...
  //fill vectors of association maps
  ...
  for (unsigned int i = 0; i < associatorMapRtoS_tsSimTS_CP.size(); ++i) {
    ...
    edm::Handle<ticl::RecoToSimCollectionSimTracksters> rToSCPHandle = event.getHandle(associatorMapRtoS_tsSimTS_CP[i]);
    recoToSimMapsLinking.push_back(*rToSCPHandle);
                                                                               
    edm::Handle<ticl::SimToRecoCollectionSimTracksters> sToRCPHandle = event.getHandle(associatorMapStoR_tsSimTS_CP[i]);
    simToRecoMapsLinking.push_back(*sToRCPHandle);
                                                                               
    //associators with SimTS from SC
    edm::Handle<ticl::RecoToSimCollectionSimTracksters> rToSSCHandle = event.getHandle(associatorMapRtoS_tsSimTS_SC[i]);
    recoToSimMapsPR.push_back(*rToSSCHandle);
                                                                     
    edm::Handle<ticl::SimToRecoCollectionSimTracksters> sToRSCHandle = event.getHandle(associatorMapStoR_tsSimTS_SC[i]);
