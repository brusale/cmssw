  ...
  for (unsigned int www = 0; www < label_tst.size(); www++) {
    ...
    if (doTrackstersPlots_) {
      ...
      histoProducerAlgo_->bookTracksterSTSHistos(
          ibook, histograms.histoProducerAlgo, BarrelVHistoProducerAlgo::validationType::Linking);
      // SimTracksters Pattern Recognition
      ibook.setCurrentFolder(dirName + "/" + label_TSToSTSPR_);
      histoProducerAlgo_->bookTracksterSTSHistos(
          ibook, histograms.histoProducerAlgo, BarrelVHistoProducerAlgo::validationType::PatternRecognition);
    }
...
void BarrelValidator::dqmAnalyze(const edm::Event& event,
  ...
  edm::Handle<std::unordered_map<DetId, const reco::PFRecHit*>> hitMapHandle;
  event.getByToken(hitMap_, hitMapHandle);
  const std::unordered_map<DetId, const reco::PFRecHit*>* hitMap = &*hitMapHandle;
