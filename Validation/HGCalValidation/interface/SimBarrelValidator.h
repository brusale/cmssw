#ifndef SimBarrelValidator_h
#define SimBarrelValidator_h
...
struct SimBarrelValidatorHistograms {
  SimBarrelVHistoProducerAlgoHistograms histoProducerAlgo;
  std::vector<dqm::reco::MonitorElement*> h_layerclusters_coll;
};

class SimBarrelValidator : public DQMGlobalEDAnalyzer<SimBarrelValidatorHistograms> {
public:
  using Histograms = SimBarrelValidatorHistograms;

  /// Constructor
  SimBarrelValidator(const edm::ParameterSet& pset);

  /// Destructor
  ~SimBarrelValidator() override;
...
protected:
  ...
  std::unique_ptr<SimBarrelVHistoProducerAlgo> histoProducerAlgo_;
