...
namespace ALPAKA_ACCELERATOR_NAMESPACE {
  ...
  class BarrelLayerClustersSoAAlgoKernelEnergy {
  public:
    ALPAKA_FN_ACC void operator()(Acc1D const& acc,
                                  ...
                                  BarrelSoAClustersDeviceCollection::View outputs) const {
      // make a strided loop over the kernel grid, covering up to "size" elements
      printf("RecHits size: %i\n", input_rechits_soa.size());
      for (int32_t i : uniform_elements(acc, input_rechits_soa.size())) {
  ...
  class BarrelLayerClustersSoAAlgoKernelPositionByHits {
  public:
    ALPAKA_FN_ACC void operator()(Acc1D const& acc,
                                  const unsigned int numer_of_clusters,
                                  float thresholdW0,
      ...
      // make a strided loop over the kernel grid, covering up to "size" elements
      for (int32_t hit_index : uniform_elements(acc, input_rechits_soa.size())) {
  ...
  class BarrelLayerClustersSoAAlgoKernelPositionByHits2 {
  public:
    ALPAKA_FN_ACC void operator()(Acc1D const& acc,
                                  const unsigned int numer_of_clusters,
                                  float thresholdW0,
      ...
      for (int32_t hit_index : uniform_elements(acc, input_rechits_soa.size())) {
        ...
        const float d1 = input_rechits_soa[hit_index].eta() - input_rechits_soa[max_energy_index].eta();
        const float d2 = input_rechits_soa[hit_index].phi() - input_rechits_soa[max_energy_index].phi();
        if (std::fmaf(d1, d1, d2 * d2) > positionDeltaRho2) {
          continue;
        }
        float Wi = input_rechits_soa[hit_index].energy();
        alpaka::atomicAdd(acc, &outputs[cluster_index].x(), input_rechits_soa[hit_index].x() * Wi);
        alpaka::atomicAdd(acc, &outputs[cluster_index].y(), input_rechits_soa[hit_index].y() * Wi);
        alpaka::atomicAdd(acc, &outputs_service[cluster_index].total_weight_log(), input_rechits_soa[hit_index].energy());
      }  // uniform_elements
    }  // operator()
  };

  // Besides the final position, add also the DetId of the seed of each cluster
  class BarrelLayerClustersSoAAlgoKernelPositionByHits3 {
  public:
    ALPAKA_FN_ACC void operator()(Acc1D const& acc,
                                  const unsigned int numer_of_clusters,
                                  float thresholdW0,
      ...
      for (int32_t cluster_index : uniform_elements(acc, outputs.metadata().size())) {
        ...
        outputs[cluster_index].x() = input_rechits_soa[max_energy_index].x();
        outputs[cluster_index].y() = input_rechits_soa[max_energy_index].y();
        outputs[cluster_index].z() = input_rechits_soa[max_energy_index].z();
      }  // uniform_elements
    }  // operator()
  };

  void BarrelLayerClustersSoAAlgoWrapper::run(Queue& queue,
    ...
    uint32_t groups = divide_up_by(input_rechits_soa.size(), items);
    ...
    alpaka::exec<Acc1D>(
        queue, workDiv, BarrelLayerClustersSoAAlgoKernelEnergy{}, size, input_rechits_soa, input_clusters_soa, outputs);
    alpaka::exec<Acc1D>(queue,
                        workDiv,
                        BarrelLayerClustersSoAAlgoKernelPositionByHits{},
                        size,
                        thresholdW0,
                        positionDeltaRho2,
                        input_rechits_soa,
    ...
    alpaka::exec<Acc1D>(queue,
                        workDiv,
                        BarrelLayerClustersSoAAlgoKernelPositionByHits2{},
                        size,
                        thresholdW0,
                        positionDeltaRho2,
                        input_rechits_soa,
    ...
    alpaka::exec<Acc1D>(queue,
                        workDivClusters,
                        BarrelLayerClustersSoAAlgoKernelPositionByHits3{},
                        size,
                        thresholdW0,
                        positionDeltaRho2,
                        input_rechits_soa,
