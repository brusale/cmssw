...
void ticl::assignPCAtoTracksters(std::vector<Trackster> &tracksters,
  ...
  for (auto &trackster : tracksters) {
    ...
    auto fillPoint = [&](const auto &c, const int idx, const float weight = 1.f) {
      point[0] = weight * c[idx].x();
      point[1] = weight * c[idx].y();
      point[2] = weight * c[idx].z();
    };
    ...
    for (size_t i = 0; i < N; ++i) {
      auto fraction = 1.f / trackster.vertex_multiplicity(i);
      int globalLayerClusterIdx = trackster.vertices(i);
      int localLayerClusterIdx = globalIdx2localIdxlayerId.at(globalLayerClusterIdx).first;
      int layerId = globalIdx2localIdxlayerId.at(globalLayerClusterIdx).second;
      auto& layerClustersView = layerClusters[layerId].view();
      trackster.addToRawEnergy(layerClustersView[localLayerClusterIdx].energy() * fraction);
      if (std::abs(layerClustersView[localLayerClusterIdx].z()) <= z_limit_em)
        trackster.addToRawEmEnergy(layerClustersView[localLayerClusterIdx].energy() * fraction);

      // Compute the weighted barycenter.
      if (energyWeight)
        weight = layerClustersView[localLayerClusterIdx].energy() * fraction;
      fillPoint(layerClustersView, localLayerClusterIdx, weight);
      for (size_t j = 0; j < 3; ++j)
        barycenter[j] += point[j];
    ...
    for (size_t i = 0; i < N; ++i) {
      ...
      int layerId = globalIdx2localIdxlayerId.at(globalLayerClusterIdx).second; 
      auto layerClustersView = layerClusters[layerId].view();
      fillPoint(layerClustersView, localLayerClusterIdx);
      if (energyWeight && trackster.raw_energy())
        weight = 
            (layerClustersView[localLayerClusterIdx].energy() / trackster.vertex_multiplicity(i)) / trackster.raw_energy();
      weights2_sum += weight * weight;
      for (size_t x = 0; x < 3; ++x)
        for (size_t y = 0; y <= x; ++y) {
          covM(x, y) += weight * (point[x] - barycenter[x]) * (point[y] - barycenter[y]);
