...
PatternRecognitionbyCLUE3D<TILES>::PatternRecognitionbyCLUE3D(const edm::ParameterSet &conf, edm::ConsumesCollector iC)
    : PatternRecognitionAlgoBaseFromSoAT<TILES>(conf, iC),
...
void PatternRecognitionbyCLUE3D<TILES>::dumpTracksters(const std::vector<std::pair<int, int>> &layerIdx2layerandSoa,
  ...
  if (PatternRecognitionAlgoBaseFromSoAT<TILES>::algo_verbosity_ > VerbosityLevel::Advanced) {
  ...
  for (auto const &t : tracksters) {
    for (auto v : t.vertices()) {
      ...
      auto layerClustersView = layerClusters[lyrIdx].view();
      auto [localIdx, layerId] = globalIdx2localIdxlayerId.at(v);
      if (PatternRecognitionAlgoBaseFromSoAT<TILES>::algo_verbosity_ > VerbosityLevel::Advanced) {
        edm::LogVerbatim("PatternRecognitionbyCLUE3D_NTP")
            ...
            << std::setw(10) << layerClustersView[localIdx].x() << sep << std::setw(10) << layerClustersView[localIdx].y() << sep
            << std::setw(10) << layerClustersView[localIdx].eta() << sep << std::setw(10) << layerClustersView[localIdx].phi() << sep
            << std::setw(10) << layerClustersView[localIdx].energy() << sep << std::setw(10) << layerClustersView[localIdx].position_error() << sep
...
void PatternRecognitionbyCLUE3D<TILES>::dumpClusters(const TILES &tiles,
  ...
  for (unsigned int layer = 0; layer < clusters_.size(); layer++) {
    ...
    auto layerClustersView = layerClusters[layer].view(); 
    int num = 0;
    for (int lcIdx = 0; lcIdx < layerClustersView.metadata().size(); ++lcIdx) {
      auto layerClusters = layerClustersView[lcIdx];
      if (PatternRecognitionAlgoBaseFromSoAT<TILES>::algo_verbosity_ > VerbosityLevel::Advanced) {
        edm::LogVerbatim("PatternRecognitionbyCLUE3D")
            << std::setw(4) << eventNumber << ", " << std::setw(3) << layer << ", " << std::setw(4)
            << thisLayer.isSeed[lcIdx] << ", " << std::setprecision(3) << std::fixed << layerClusters.x() << layerClusters.y()
            << ", " << layerClusters.z() << ", " << layerClusters.r_over_absz() << ", " << layerClusters.eta() << ", "
            << layerClusters.phi() << ", " << std::setw(5) << tiles[layer].etaBin(layerClusters.eta()) << ", "
            << std::setw(5) << tiles[layer].phiBin(layerClusters.phi()) << ", " << std::setw(4) << layerClusters.cells()
            << ", " << std::setprecision(3) << layerClusters.energy() << ", "
            << (layerClusters.energy() / thisLayer.rho[lcIdx]) << ", " << thisLayer.rho[lcIdx] << ", " 
            << thisLayer.z_extension[lcIdx] << ", " << std::scientific << thisLayer.delta[lcIdx].first << ", " 
            << std::setw(10) << thisLayer.delta[lcIdx].second << ", " << std::setw(5)
            << thisLayer.nearestHigher[lcIdx].first << ", " << std::setw(10) << thisLayer.nearestHigher[lcIdx].second
            << ", " << std::defaultfloat << std::setprecision(3) << layerClusters.position_error() << ", " << std::setw(5)
            << thisLayer.clusterIndex[lcIdx] << ", " << std::setw(4) << thisLayer.layerClusterOriginalIdx[lcIdx] << ", "
            << std::setw(4) << lcIdx << ", ClusterInfo";
      }
    }
  }
...
void PatternRecognitionbyCLUE3D<TILES>::makeTracksters(
    const typename PatternRecognitionAlgoBaseFromSoAT<TILES>::Inputs &input,
  ...
  std::unordered_map<int, std::pair<int, int>> globalIdx2localIdxlayerId; // used for PCA calculation
  /*auto nLayerClusters = std::accumulate(input.layerClusters.begin(), input.layerClusters.end(), 0, 
                                        [](auto size, auto el) {
                                            auto view = el.view();
                                            return size += view.metadata().size();
                                        });*/
  unsigned int nLayerClusters = 0;
  for (auto& clustersOnLayer : input.layerClusters) {
    auto clustersOnLayerView = clustersOnLayer.view();
    nLayerClusters += clustersOnLayerView.metadata().size();
  }

  layerIdx2layerandSoa.reserve(nLayerClusters);
  unsigned int layerIdx = 0;
  //for (auto const &lc : input.layerClusters) {
  for (unsigned int layerId = 0; layerId < input.layerClusters.size(); ++layerId) {
    auto layerClustersView = input.layerClusters[layerId].view();
    if (input.mask[layerId] == 0.) {
      if (PatternRecognitionAlgoBaseFromSoAT<TILES>::algo_verbosity_ > VerbosityLevel::Advanced) {
    ...
    for (int i = 0; i < layerClustersView.metadata().size(); i++) {
      layerIdx2layerandSoa.emplace_back(layerId, clusters_[layerId].isSilicon.size());
      globalIdx2localIdxlayerId.emplace(std::make_pair(layerIdx, std::make_pair(i, layerId)));
      auto detId = layerClustersView[0].seed();
      clusters_[layerId].isSilicon.push_back(rhtools_.isSilicon(detId));
      clusters_[layerId].isSeed.push_back(false);
      clusters_[layerId].clusterIndex.push_back(-1);
      clusters_[layerId].layerClusterOriginalIdx.emplace_back(layerIdx++);
      clusters_[layerId].nearestHigher.emplace_back(-1, -1);
      clusters_[layerId].rho.emplace_back(0.f);
      clusters_[layerId].z_extension.emplace_back(0.f);
      clusters_[layerId].delta.emplace_back(
          std::make_pair(std::numeric_limits<float>::max(), std::numeric_limits<int>::max()));
    }
  }
  for (unsigned int layer = 0; layer < clusters_.size(); layer++) {
    clusters_[layer].followers.resize(clusters_[layer].isSilicon.size());
  }
  ...
  for (int i = 0; i <= maxLayer; i++) {
    calculateLocalDensity(input.tiles, input.layerClusters, i, layerIdx2layerandSoa);
  }
  for (int i = 0; i <= maxLayer; i++) {
    calculateDistanceToHigher(input.tiles, input.layerClusters, i, layerIdx2layerandSoa);
  }

  auto nTracksters = findAndAssignTracksters(input.tiles, input.layerClusters, layerIdx2layerandSoa);
  if (PatternRecognitionAlgoBaseFromSoAT<TILES>::algo_verbosity_ > VerbosityLevel::Advanced) {
    edm::LogVerbatim("PatternRecognitionbyCLUE3D") << "Reconstructed " << nTracksters << " tracksters" << std::endl;
    dumpClusters(input.tiles, input.layerClusters, layerIdx2layerandSoa, eventNumber);
  }
  ...
  for (unsigned int layer = 0; layer < clusters_.size(); ++layer) {
    const auto &clustersOnLayer = clusters_[layer];
    const auto &layerClustersOnLayer = input.layerClusters[layer].view();
    if (PatternRecognitionAlgoBaseFromSoAT<TILES>::algo_verbosity_ > VerbosityLevel::Advanced) {
      edm::LogVerbatim("PatternRecognitionbyCLUE3D") << "Examining Layer: " << layer;
    }
    for (unsigned int lc = 0; lc < clustersOnLayer.rho.size(); ++lc) {
      if (PatternRecognitionAlgoBaseFromSoAT<TILES>::algo_verbosity_ > VerbosityLevel::Advanced) {
        edm::LogVerbatim("PatternRecognitionbyCLUE3D") << "Trackster " << clustersOnLayer.clusterIndex[lc];
      }
      if (clustersOnLayer.clusterIndex[lc] >= 0) {
        if (PatternRecognitionAlgoBaseFromSoAT<TILES>::algo_verbosity_ > VerbosityLevel::Advanced) {
          edm::LogVerbatim("PatternRecognitionbyCLUE3D") << " adding lcIdx: " << clustersOnLayer.layerClusterOriginalIdx[lc];
        }
        result[clustersOnLayer.clusterIndex[lc]].vertices().push_back(clustersOnLayer.layerClusterOriginalIdx[lc]);
        result[clustersOnLayer.clusterIndex[lc]].vertex_multiplicity().push_back(1);
        // loop over followers
        for (auto [follower_lyrIdx, follower_soaIdx] : clustersOnLayer.followers[lc]) {
          std::array<unsigned int, 2> edge = {
              {(unsigned int)clustersOnLayer.layerClusterOriginalIdx[lc],
               (unsigned int)clusters_[follower_lyrIdx].layerClusterOriginalIdx[follower_soaIdx]}};
          result[clustersOnLayer.clusterIndex[lc]].edges().push_back(edge);
        }
      }
    }
  }
  ...
  ticl::assignPCAtoTracksters(result,
                              input.layerClusters,
                              globalIdx2localIdxlayerId, 
                              input.layerClustersTime,
                              rhtools_.getPositionLayer(rhtools_.lastLayerEE(false), false).z());

  // run energy regression and ID
  energyRegressionAndID(input.layerClusters, globalIdx2localIdxlayerId, input.tfSession, result);
  if (PatternRecognitionAlgoBaseFromSoAT<TILES>::algo_verbosity_ > VerbosityLevel::Advanced) {
  ...
  if (PatternRecognitionAlgoBaseFromSoAT<TILES>::algo_verbosity_ > VerbosityLevel::Advanced) {
    dumpTracksters(layerIdx2layerandSoa, globalIdx2localIdxlayerId, input.layerClusters, eventNumber, result);
  }
...
template <typename TILES>
void PatternRecognitionbyCLUE3D<TILES>::energyRegressionAndID(const std::vector<LayerClustersCollection> &layerClusters,
                                                              const std::unordered_map<int, std::pair<int, int>> &globalIdx2localIdxlayerId,
  ...
  for (int i = 0; i < static_cast<int>(tracksters.size()); i++) {
    ...
    for (const unsigned int &vertex : tracksters[i].vertices()) {
      auto [localIdx, layerId] = globalIdx2localIdxlayerId.at(vertex);
      auto& layerClustersView = layerClusters[layerId].view();
      sumClusterEnergy += layerClustersView[localIdx].energy();
  ...
  for (int i = 0; i < batchSize; i++) {
    ...
    sort(clusterIndices.begin(), clusterIndices.end(), [&](const int &a, const int &b) {
      auto [localIdx_a, layerId_a] = globalIdx2localIdxlayerId.at(a);
      auto [localIdx_b, layerId_b] = globalIdx2localIdxlayerId.at(b);
      
      auto layerClustersView_a = &(layerClusters[layerId_a].view());
      auto layerClustersView_b = &(layerClusters[layerId_b].view());
      
      return layerClustersView_a[localIdx_a].energy() > layerClustersView_b[localIdx_b].energy();
    });
    ...
    for (const int &k : clusterIndices) {
      auto [localIdx, layerId] = globalIdx2localIdxlayerId.at(k);
      auto& layerClustersView = layerClusters[layerId].view();
      // get features per layer and cluster and store the values directly in the input tensor
      //int j = rhtools_.getLayerWithOffset(cluster.hitsAndFractions()[0].first) - 1;
      if (layerId < eidNLayers_ && seenClusters[layerId] < eidNClusters_) {
        // get the pointer to the first feature value for the current batch, layer and cluster
        float *features = &input.tensor<float, 4>()(i, layerId, seenClusters[layerId], 0);

        // fill features
        *(features++) = float(layerClustersView[localIdx].energy() / float(trackster.vertex_multiplicity(k)));
        *(features++) = float(std::abs(layerClustersView[localIdx].eta()));
        *(features) = float(layerClustersView[localIdx].phi());

        // increment seen clusters
        seenClusters[localIdx]++;
      }
    }
...
void PatternRecognitionbyCLUE3D<TILES>::calculateLocalDensity(
    const TILES &tiles, const std::vector<LayerClustersCollection> &layerClusters, const int layerId, const std::vector<std::pair<int, int>> &layerIdx2layerandSoa) {
  constexpr int nEtaBin = TILES::constants_type_t::nEtaBins;
  constexpr int nPhiBin = TILES::constants_type_t::nPhiBins;
  auto &clustersOnLayer = clusters_[layerId];
  auto &layerClustersOnLayer = layerClusters[layerId].view();
  int numberOfClusters = clustersOnLayer.isSilicon.size();
  ...
  for (int32_t i = 0; i < numberOfClusters; i++) {
    ...
    for (int currentLayer = minLayer; currentLayer <= maxLayer; currentLayer++) {
      ...
      int etaBinMin = std::max(tileOnLayer.etaBin(layerClustersOnLayer[i].eta()) - etaWindow, 0);
      int etaBinMax = std::min(tileOnLayer.etaBin(layerClustersOnLayer[i].eta()) + phiWindow, nEtaBin - 1);
      int phiBinMin = tileOnLayer.phiBin(layerClustersOnLayer[i].phi()) - phiWindow;
      int phiBinMax = tileOnLayer.phiBin(layerClustersOnLayer[i].phi()) + phiWindow;
      if (PatternRecognitionAlgoBaseFromSoAT<TILES>::algo_verbosity_ > VerbosityLevel::Advanced) {
        edm::LogVerbatim("PatternRecognitionbyCLUE3D") << "eta: " << layerClustersOnLayer[i].eta();
        edm::LogVerbatim("PatternRecognitionbyCLUE3D") << "phi: " << layerClustersOnLayer[i].phi();
      ...
      for (int ieta = etaBinMin; ieta <= etaBinMax; ++ieta) {
        ...
        for (int iphi_it = phiBinMin; iphi_it <= phiBinMax; ++iphi_it) {
          ...
          for (auto otherClusterIdx : tileOnLayer[offset + iphi]) {
            ...
            auto const &layerClustersOnOtherLayer = layerClusters[layerandSoa.first].view();
            if (PatternRecognitionAlgoBaseFromSoAT<TILES>::algo_verbosity_ > VerbosityLevel::Advanced) {
              edm::LogVerbatim("PatternRecognitionbyCLUE3D")
                  << "OtherLayer: " << layerandSoa.first << " SoaIDX: " << layerandSoa.second;
              edm::LogVerbatim("PatternRecognitionbyCLUE3D") << "OtherEta: " << layerClustersOnOtherLayer[layerandSoa.second].eta();
              edm::LogVerbatim("PatternRecognitionbyCLUE3D") << "OtherPhi: " << layerClustersOnOtherLayer[layerandSoa.second].phi();
            }

            bool onSameCluster = layerClustersOnLayer[i].layerId() == static_cast<int>(otherClusterIdx);
            ...
            if (useAbsoluteProjectiveScale_) {
              if (useClusterDimensionXY_) {
                reachable = isReachable(layerClustersOnLayer[i].r_over_absz() * layerClustersOnLayer[i].z(),
                                        layerClustersOnOtherLayer[layerandSoa.second].r_over_absz() * layerClustersOnOtherLayer[i].z(),
                                        layerClustersOnLayer[i].phi(),
                                        layerClustersOnOtherLayer[layerandSoa.second].phi(),
                                        layerClustersOnLayer[i].position_error() * layerClustersOnLayer[i].position_error());
              } else {
              ...
              reachable = (reco::deltaR2(layerClustersOnLayer[i].eta(),
                                         layerClustersOnLayer[i].phi(),
                                         layerClustersOnOtherLayer[layerandSoa.second].eta(),
                                         layerClustersOnOtherLayer[layerandSoa.second].phi()) < densityEtaPhiDistanceSqr_);
            }
            if (PatternRecognitionAlgoBaseFromSoAT<TILES>::algo_verbosity_ > VerbosityLevel::Advanced) {
              edm::LogVerbatim("PatternRecognitionbyCLUE3D") << "Distance[eta,phi]: "
                                                             << reco::deltaR2(layerClustersOnLayer[i].eta(),
                                                                              layerClustersOnLayer[i].phi(),
                                                                              layerClustersOnOtherLayer[layerandSoa.second].eta(),
                                                                              layerClustersOnOtherLayer[layerandSoa.second].phi());
              auto dist = distance_debug(
                  layerClustersOnLayer[i].r_over_absz(),
                  layerClustersOnOtherLayer[layerandSoa.second].r_over_absz(),
                  layerClustersOnLayer[i].r_over_absz() * std::abs(layerClustersOnLayer[i].phi()),
                  layerClustersOnOtherLayer[layerandSoa.second].r_over_absz() * std::abs(layerClustersOnOtherLayer[layerandSoa.second].phi()));
              edm::LogVerbatim("PatternRecognitionbyCLUE3D") << "Distance[cm]: " << (dist * layerClustersOnLayer[i].z());
              edm::LogVerbatim("PatternRecognitionbyCLUE3D")
                  << "Energy Other:   " << layerClustersOnOtherLayer[layerandSoa.second].energy();
              edm::LogVerbatim("PatternRecognitionbyCLUE3D") << "Cluster radius: " << layerClustersOnLayer[i].position_error();
            }
            if (reachable) {
              auto energyToAdd = 
                  (onSameCluster ? 1.f : kernelDensityFactor_) * layerClustersOnOtherLayer[layerandSoa.second].energy();
  ...
  for (int i = 0; i < numberOfClusters; i++) {
    if (PatternRecognitionAlgoBaseFromSoAT<TILES>::algo_verbosity_ > VerbosityLevel::Advanced) {
      edm::LogVerbatim("PatternRecognitionbyCLUE3D")
          ...
          << " at eta, phi: " << tiles[layerId].etaBin(layerClustersOnLayer[i].eta()) << ", "
          << tiles[layerId].phiBin(layerClustersOnLayer[i].phi());
    }
    // We need to partition the two sides of the HGCAL detector
    auto lastLayerPerSide = static_cast<int>(rhtools_.lastLayer(false));
    int minLayer = 0;
    ...
    for (int currentLayer = minLayer; currentLayer <= maxLayer; currentLayer++) {
      ...
      int phiWindow = 1;
      int etaBinMin = std::max(tileOnLayer.etaBin(layerClustersOnLayer[i].eta()) - etaWindow, 0);
      int etaBinMax = std::max(tileOnLayer.etaBin(layerClustersOnLayer[i].eta()) + etaWindow, nEtaBin - 1);
      int phiBinMin = tileOnLayer.phiBin(layerClustersOnLayer[i].phi()) - phiWindow;
      int phiBinMax = tileOnLayer.phiBin(layerClustersOnLayer[i].phi()) + phiWindow;
      for (int ieta = etaBinMin; ieta <= etaBinMax; ++ieta) {
        auto offset = ieta * nPhiBin;
        for (int iphi_it = phiBinMin; iphi_it <= phiBinMax; ++iphi_it) {
          ...
          for (auto otherClusterIdx : tileOnLayer[offset + iphi]) {
            ...
            if (useAbsoluteProjectiveScale_) {
              dist_transverse = distanceSqr(layerClustersOnLayer[i].r_over_absz() * layerClustersOnLayer[i].z(),
                                            layerClustersOnOtherLayer[layerandSoa.second].r_over_absz() * layerClustersOnLayer[i].z(),
                                            layerClustersOnLayer[i].phi(),
                                            layerClustersOnOtherLayer[layerandSoa.second].phi());
              // Add Z-scale to the final distance
              dist = dist_transverse;
            } else {
              dist = reco::deltaR2(layerClustersOnLayer[i].eta(),
                                   layerClustersOnLayer[i].phi(),
                                   layerClustersOnOtherLayer[layerandSoa.second].eta(),
                                   layerClustersOnOtherLayer[layerandSoa.second].phi());
...
int PatternRecognitionbyCLUE3D<TILES>::findAndAssignTracksters(
    const TILES &tiles, const std::vector<LayerClustersCollection> &layerClusters, const std::vector<std::pair<int, int>> &layerIdx2layerandSoa) {
  ...
  for (unsigned int layer = 0; layer < 2 * rhtools_.lastLayer(); layer++) {
    ...
    auto &layerClustersOnLayer = layerClusters[layer].view();
    unsigned int numberOfClusters = clustersOnLayer.isSilicon.size();
    for (unsigned int i = 0; i < numberOfClusters; i++) {
      ...
      bool isSeed = (clustersOnLayer.delta[i].first > critical_transverse_distance ||
                    ...
                    (layerClustersOnLayer[i].energy() / clustersOnLayer.rho[i] > criticalSelfDensity_);
      if (!clustersOnLayer.isSilicon[i]) {
        isSeed = (clustersOnLayer.delta[i].first > layerClustersOnLayer[i].position_error() ||
                  clustersOnLayer.delta[i].second > criticalZDistanceLyr_) &&
                 (clustersOnLayer.rho[i] >= criticalDensity_) &&
                 (layerClustersOnLayer[i].energy() / clustersOnLayer.rho[i] > criticalSelfDensity_);
      }
